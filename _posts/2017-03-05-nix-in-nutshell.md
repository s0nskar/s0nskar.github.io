...
### Data Models
NIX avail various types of data models for storing data. Each of them has `id, name, type and definition`. Further, it assumes a central role in the linking between data and metadata. Few types of Data models are briefed below.
...
+ ##### Block:
    A Block is a top level grouping element for data objects that somehow are related to each other. It is a requirement that every data object has to be associated with one Block object.
+ ##### Group:
    The Group establishes subgroups below the Block while entities can be part of multiple Groups. A Group can contain DataArrays, Tags, and MultiTags. It further allows linking to Sources and attaching metadata.
...
### What Now?
I am currently reading NIX code to get the further understanding how various Data Model are implemented deep down. My primary interest is in the implementation of DataArray. As DataFrame implementation is gonna be almost similar to them. I will write a new blog post explaining how I am gonna implement POC for DataFrame.
